...
Per-VM `auth_token` is an HMAC-SHA256 provisioning token minted by `ConfigResolver._mint_provisioning_token()` when both `server_url` and `spec` are set. See [provisioning-token.md](provisioning-token.md) for token format, signing, and verification.
